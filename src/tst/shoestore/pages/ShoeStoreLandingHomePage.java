...
public class ShoeStoreLandingHomePage {
	...
	/*The Check method will check the email remainder functionality of the 
	 * Shoe Store by entering various inputs.
	 *  
	 */
	public static String check(WebDriver driver) {
		...
		if(waitForPresence(wait,emailRemainderXpath())){
			if(!driver.findElement(emailRemainderMsg()).
					getText().
					equalsIgnoreCase("Please enter an email address")){
				return "Empty Email not proper";
			}
		}else{
			return "Unable to check for no input";
		}
		...
		driver.findElement(emailRemainderSubmitBtnk()).click();
		wait.until(ExpectedConditions.visibilityOfElementLocated(emailRemainderMsg()));
		System.out.println(driver.findElement(emailRemainderMsg()).
				getText());
		if(!driver.findElement(emailRemainderMsg()).
				getText().
	...
	/*This method checks the if the email remainder is available and has enough area will return false if the field
	 * not available or area is not enough else will return true
	 */
	...
	/*This method will Explicitly wait for wait(sec) for the presence of the element provided 
	 * provided by xpath locator 
	 */
	...
	/*This method will Explicitly wait for wait(sec) for the invisibility of the element provided 
	 * provided by xpath locator 
	 */
	public static boolean waitForInvisibility(WebDriverWait wait, String xPath){
		try{
			...
			//The TimeoutException the is generated for non availability of the element is supressed
			return false;
		}
